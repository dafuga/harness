...
export function auditCurlyFunctions(path: string, lines: string[], startsFunction: (line: string) => boolean): AuditFinding[] {
	return collectCurlyBlocks(lines, startsFunction).flatMap((block) => auditFunctionBlock(path, block));
}
...
function collectCurlyBlocks(lines: string[], startsFunction: (line: string) => boolean): BlockCandidate[] {
...
function collectIndentedBlocks(lines: string[]): BlockCandidate[] {
	const blocks: BlockCandidate[] = [];
	lines.forEach((line, index) => {
		if (/^\s*def\s+\w+/.test(line)) blocks.push({ start: index, lines: pythonBlockLines(lines, index) });
	});
	return blocks;
}
...
function auditBlockLength(path: string, block: BlockCandidate): AuditFinding[] {
	if (block.lines.length <= frameRuleLimits.maxFunctionLines) return [];
