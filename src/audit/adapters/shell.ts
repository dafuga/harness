	...
	return [
		{
			...
			rule: 'shell-shebang',
			message: 'Shell scripts should start with a shebang.'
		}
	];
}
	...
	return [
		{
			...
			message: 'Shell scripts should use set -euo pipefail.'
		}
	];
}
...
function auditUnsafeRm(file: AuditFile) {
	...
	return [
		{ path: file.relativePath, rule: 'shell-unsafe-rm', message: 'Avoid broad rm -rf patterns.' }
	];
}

function auditUnquotedVariable(file: AuditFile) {
	if (!file.lines.some((line) => /\s\$\w+/.test(line) && !/"[^"]*\$\w+[^"]*"/.test(line)))
		return [];
	return [
		{
			path: file.relativePath,
			rule: 'shell-unquoted-variable',
			message: 'Quote shell variables in command arguments.'
		}
	];
}
