...
test('generated app audit rejects TypeScript, Svelte, and SQL rule violations', async () => {
	const root = await mkdtemp(join(tmpdir(), 'frame-app-adapters-'));
	try {
		await runFrame(['new', 'app', 'adapter-app'], root);
		...
		expectRules(commandOutput(auditFailure), ['svelte-script-lang', 'svelte-component-name', 'sql-dangerous-migration']);
...
test('generated lib audit rejects C++, Python, shell, and WASM rule violations', async () => {
	const root = await mkdtemp(join(tmpdir(), 'frame-lib-adapters-'));
	try {
		await runFrame(['new', 'lib', 'adapter-lib'], root);
...
async function writeAppViolations(project: string): Promise<void> {
	...
	await writeFile(join(project, 'src/components/bad-name.svelte'), '<script>let label = "Bad";</script>\n<h1>{label}</h1>\n');
