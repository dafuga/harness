...
const ruleFixtures: Record<string, string> = {
	'small-class': ['export class BigClass {', ...Array.from({ length: 121 }, (_, index) => `\tvalue${index} = ${index};`), '}'].join('\n'),
	'method-length': ['export class BigMethod {', '\trun(): void {', ...Array.from({ length: 36 }, () => '\t\tvoid 1;'), '\t}', '}'].join('\n'),
...
test('audit reports architecture rule violations', async () => {
	...
	await writeFile(join(root, 'src/core/bad.ts'), "import { projectFiles } from '../templates/project';\n");
	await writeFile(join(root, 'src/commands/bad.ts'), "import { projectFiles } from '../templates/project';\n");
